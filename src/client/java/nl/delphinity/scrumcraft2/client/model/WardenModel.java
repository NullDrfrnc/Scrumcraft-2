package nl.delphinity.scrumcraft2.client.model;
...
public class WardenModel extends EntityModel<WardenRenderState> {
    ...
    private void animateIdlePose(float f) {
        ...
        var10000.zRot += 0.06F * h;
        var10000 = this.head;
        var10000.xRot += 0.06F * i;
        var10000 = this.body;
        var10000.zRot += 0.025F * i;
        var10000 = this.body;
        var10000.xRot += 0.025F * h;
    }

    private void animateWalk(float f, float g) {
        float h = Math.min(0.5F, 3.0F * g);
        float i = f * 0.8662F;
        float j = Mth.cos((double)i);
        float k = Mth.sin((double)i);
        float l = Math.min(0.35F, h);
        ModelPart var10000 = this.head;
        var10000.zRot += 0.3F * k * h;
        var10000 = this.head;
        var10000.xRot += 1.2F * Mth.cos((double)(i + ((float)Math.PI / 2F))) * l;
        this.body.zRot = 0.1F * k * h;
        this.body.xRot = 1.0F * j * l;
        this.leftLeg.xRot = 1.0F * j * h;
        this.rightLeg.xRot = 1.0F * Mth.cos((double)(i + (float)Math.PI)) * h;
    ...
    private void animateTendrils(WardenRenderState wardenRenderState, float f) {
        float g = wardenRenderState.tendrilAnimation * (float)(Math.cos((double)f * (double)2.25F) * Math.PI * (double)0.1F);
